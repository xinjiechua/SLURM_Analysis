...
public class exit0GraphPanel extends JPanel{
	...
	public void paint(Graphics g) {
		...
		String title = "Number of jobs completed with exit status 0";
		int scaleYnum = 4;
		int scaleYdiff = 300;
		int scaleYini = 0;
		int mult = 2;
		...
		//set data in y axis
		int[] yVar = {1292,779,782,821,1087,687,331};
		...
		for(int i = 0; i<scaleYnum; i++) {
			...
			g2D.drawString(scalee, leftBorder-50, yAxislength+upBorder-labelspace*mult*(i+1)+10);
		}

		for(int i = 0; i<scaleYnum; i++) {
			g2D.drawLine(leftBorder-labellength/2, yAxislength+upBorder-labelspace*mult*(i+1), leftBorder-labellength/2+labellength, yAxislength+upBorder-labelspace*mult*(i+1));
		}
		...
		for(int i = 0; i<xAxisnum-1; i++) {
			g2D.drawLine(space*(i+1)+leftBorder, yAxislength-yVar[i]*mult/6+upBorder,space*(i+2)+leftBorder ,yAxislength-yVar[(i+1)]*mult/6+upBorder);
		}

		//draw dots on graph -----------------------
		g2D.setColor(new Color(195,172,180));
		for(int i = 0; i<xAxisnum; i++) {
			g2D.fillOval(space*(i+1)+leftBorder-5, yAxislength-yVar[i]*mult/6+upBorder-5, 10, 10);
		}
